...
import com.enigma.api.inventory.exceptions.AplicationException;
...
public class DefaultExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    private ResponseEntity<Object> handleBindingResult(BindingResult result, HttpStatus status) {
        ...
        ResponsMessage<Object> body = ResponsMessage.error(status.value(), message, errors);
    ...
    public ResponseEntity<Object> handlerApplicationException(AplicationException e) {
        return handlerException(e.getStatus(), e.getMessage());
    }

    private ResponseEntity<Object> handlerException(HttpStatus status, String message) {
        String localizedMessage = messageSource.getMessage(message, null, message, LocaleContextHolder.getLocale());
        ResponsMessage<Object> body = ResponsMessage.error(status.value(), localizedMessage);
        return ResponseEntity.ok(body);
    }
    ...
    public ResponseEntity<Object> handleUnknownException(Exception e) {
        ...
        return handlerException(HttpStatus.INTERNAL_SERVER_ERROR);
    }

    private ResponseEntity<Object> handlerException(HttpStatus status) {
        String message = "error." + status.value();
        return handlerException(status, message);
    }

    @Override
    protected ResponseEntity<Object> handleExceptionInternal(Exception ex, Object body, HttpHeaders headers, HttpStatus status, WebRequest request) {
        return handlerException(status);
    }
